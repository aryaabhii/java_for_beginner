...
// prg to add two matrices whose row and column of both are same suing Function..........
import java.util.*;

class DiagonalSumMatrixFunction {

    void Diago(int matrix[][]) {
        // below code for printing the calculated sum which is stored in sum var.....
        System.out.println("Sum of diagonal matrices with [" + matrix.length + "] rows & [" + matrix.length
                + "] column are : ");
        int sum = 0;
        for (int i = 0; i < matrix.length; i++) { // for row..
            for (int j = 0; j < matrix[i].length; j++) { // for column...
                if (i == j)
    ...
    public static void main(String args[]) {
        ...
        // checking condtion for row and column.................................
        if (row == col) {
        ...
        for (int i = 0; i < matrix.length; i++) { // for row value...
            for (int j = 0; j < matrix[i].length; j++) { // for column value...
                System.out.print(matrix[i][j] + "\t"); // printing the value of matrix.....
            }
            System.out.println();
        }
        DiagonalSumMatrixFunction obj = new DiagonalSumMatrixFunction(); // object creation using class.....
        obj.Diago(matrix); // function call......
        sc.close();
    }
}